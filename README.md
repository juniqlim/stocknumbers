...
- **003230 (삼양식품) / 214450 (파마리서치)** — KRW, DART API(`fnlttSinglAcntAll`, CIS)로 연간 2017~ 직접 수집. 분기는 누적 차분. 003230은 `_extra`에 **판관비 표(2017~2025)**: 사업보고서 주석 '판매비와관리비'(연결, 당기·전기 2개년/보고서 → 2018·2020·2022·2024·2025 보고서로 커버) 파싱, 광고선전비·물류비(운반비+수출제비용)와 매출 대비 비율. 검증: 매출총이익−판관비≈영업이익.
...
- **NOW (서비스나우)** — 미국주식(USD, `십억$`). EDGAR API 기반, 연간(2017~2025)·분기(Q1'24~Q2'26)·재무상태표 + 시총, `_extra`에 **RPO(잔여계약가치)·이연수익** 분기표. 엔터프라이즈 SaaS 표준 예시 — 매출은 20%+ 꾸준히, GAAP 순이익은 주식보상비로 얇아 PER이 극단적(2020 937배). 함정: ①**2025-12-17 5:1 액면분할** → FY2025 10-K(2026-01-29 제출)가 과거를 소급 재표시해 EDGAR frame에 분할전/후가 섞임. 2024년 이전 주식수는 ×5, EPS는 ÷5로 분할후 통일(야후 종가는 이미 분할반영). ②**ASC606 소급적용**으로 2016~2017이 재표시됨 → 2018-02 제출본이 아니라 **2019-02 제출본 값**을 써야 매출·GPM 기준이 맞는다(`Revenues` 대신 `RevenueFromContractWithCustomerExcludingAssessedTax`). ③2019 순이익 급증은 이연법인세 평가충당금 환입(약 4.7억$) 일회성. ④2026년 M&A 러시(Moveworks 24억$·Veza 12억$·Armis 76억$)로 무형자산 상각비가 25→219백만$(분기)로 뛰며 GPM·OPM 급락 — Q2'26 OPM 4.1%. ⑤구독매출은 축(axis) 태깅이라 companyfacts에 안 잡힘 → 총매출만 사용. 무배당이라 배당률 열 없음.
...
- **023530 (롯데쇼핑)** — KRW 오프라인 유통(백화점·마트·슈퍼·하이마트·이커머스·컬처웍스). DART API(`fnlttSinglAcntAll`, CFS) 연간 2017~2025 + 분기 + 재무상태표 + 시총. 손익=CIS, 매출 `ifrs-full_Revenue`. 유통업이라 **GPM(판관비 차감 전) 42~48%인데 OPM은 1~4%** — 격차가 판관비(인건비·임차료·감가상각). 함정: ①**2017~2022 EPS가 계속영업/중단영업으로 분리 공시**(`ifrs-full_BasicEarningsLossPerShareFromContinuingOperations` + `...FromDiscontinuedOperations` 합산; 배당공시 `alotMatter`의 (연결)주당순이익으로 교차검증). ②**1Q24 CAPEX가 음수 부호로 공시** → 누적 차분 전에 `abs()`. ③2019 IFRS16 도입으로 자산·부채 급증(부채비율 111%→188%). ④**2024 유형자산 재평가**(기타포괄손익 +9.4조)로 자산 30.6조→39.0조·자본 10.8조→17.0조, 같은 해 손상차손 등으로 4Q24 순손실 -9,550억. ⑤FCF에 리스부채 상환(연 6천억+)이 재무활동이라 미차감. 2017년만 발행주식 28,122,047주(이후 28,288,755주). 매출은 2017 17.9조 → 2025 13.7조로 8년 연속 역성장이나 OPM·순이익은 2025~2026 회복, PBR 0.2배.
- **020120 (키다리스튜디오)** — KRW 웹툰 플랫폼(봄툰·레진코믹스·델리툰, IP 굿즈·AI 챗봇). DART API(`fnlttSinglAcntAll`, CFS) 연간 2017~2025 + 분기(1Q24~2Q26, 손익·CF 모두 누적 차분) + 재무상태표 + 시총. 손익=CIS. 함정: ①2017~2019 CF에 `유형자산의 취득` 계정이 없고 `집기비품의 취득`만 있음 → 그걸 CAPEX로 사용. ②콘텐츠·소프트웨어(무형자산) 취득이 유형 CAPEX보다 큼(2025 86억 vs 7억) → FCF가 관례상 과대, 캡션에 명시. ③2021 레진엔터테인먼트 주식교환 인수로 주식수 18.5M→34.7M·자산 6배·영업권 급증, 2023 영업권 손상 등 253억으로 -345억 순손실. ④2017 지배지분 미공시 → 자본총계로 ROE. 배당은 2022(0.60%) 한 해뿐. 2024 매출 +20%·2025 OPM 5.7%·1H26 영업이익 +275%로 턴어라운드 중.
